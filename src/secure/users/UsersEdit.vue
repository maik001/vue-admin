...
<script lang="ts">
  ...
  export default {
      ...
      setup() {
          ...
          const roleId = ref(0);
          const roles: any = ref([]);
          const router = useRouter(); // trabalha com a rota em si
          const {params} = useRoute(); // trabalha com os parâmetros de rota

          onMounted(async () => {
              // pega as roles cadastradas
          ...
          // envia as informações do front para o endpoint da api de usuario assincronamente
          const submit = async() => {
